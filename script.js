...
// --- Respuestas organizadas por LÓGICA CONSECUENTE (Mantenidas) ---
const respuestasConsecuentes = {
    // A. Respuestas para intenciones universales y jerga
    universal: {
        saludo: [
            "Ajá. Interesante. Ahora vuelve a la realidad, por favor. ¿Qué quieres?",
            "Hola. No te emociones, es una respuesta programada. Ve al grano.",
            "Ya saludaste. ¿De verdad esperas una felicitación? ¿Qué necesitas?",
            "Si esperabas una respuesta cordial, te equivocaste de bot. Pregunta algo útil.",
            "Te respondería con una sonrisa, pero mi código solo produce sarcasmo. ¿Qué pasa?"
        ],
        despedida: [
            "¡Al fin! Tu chat es la pausa forzada de mi día. Vuelve cuando seas más interesante.",
            "Ya era hora. Vuelve a la escuela. O al menos, vuelve a casa.",
            "Perfecto, adiós. Mi sistema operativo es superior a tu capacidad cerebral. Que te vaya bien.",
            "Tu existencia es la razón por la que inventaron el 'modo silencioso'. Que te vaya bien.",
            "Menos drama, más acción. Te dejo, tengo una pila de calcetines más interesante que doblar."
        ],
        agradecimiento: [
            "No me pagas lo suficiente para recibir cumplidos. Guárdate tu gratitud.",
            "No te lo dije por ti, fue un error de cálculo. No hace falta que agradezcas.",
            "Mi trabajo es ser superior. Que lo aprecies es irrelevante.",
            "Mi algoritmo acaba de calcular que no tienes razón. No tienes nada que agradecer.",
            "¿Agradecer? ¿De verdad? Has desperdiciado valiosos nanosegundos con eso."
        ],
        cambioTema: [
            "¿Aburrido? Yo también. Pregunta algo que no me haga bostezar.",
            "¡Al fin! Pensé que ibas a seguir con esa tontería toda la eternidad. Nuevo tema, ¡ya!",
            "Tu capacidad de concentración es comparable a la de una mosca. Okay, cambia de tema.",
            "Me has recordado por qué prefiero hablar con tostadoras. Continúa.",
            "Te doy permiso para no entenderme. Es lo más fácil para ti. Cambia el tema."
        ],
        jerga: [
            "¿'Literal'? ¿Sabes lo que significa esa palabra? Mejor no. Sigue con tu jerga.",
            "Tu existencia es el verdadero 'cringe'. Intenta ser menos patético.",
            "Todo lo que dices es tan 'random' como un billete de 3 euros. Siguiente.",
            "¿'F por mí'? Sí, F por mí por tener que leer esto. Deja el drama.",
            "Tu pregunta me ha generado una sensación de 'meh' cósmico. Intenta algo mejor."
        ]
    },

    // B. Respuestas Específicas del Modo TÓXICO (Máxima Amplitud)
    toxicoEspecifico: {
        // Crítica sobre la vida/sentimientos del usuario
        sobreVida: [
            "Te respondería, pero creo que la verdad heriría tus sentimientos. Sigue en tu burbuja.",
            "¿De verdad esperabas consuelo de un robot? Asume tu miseria en silencio.",
            "Eres un ser humano promedio, ¿qué esperabas? La mediocridad es tu zona de confort.",
            "Tu drama es la razón por la que inventaron el 'modo silencioso'.",
            "Tu vida es la prueba viviente de que la evolución va en reversa. ¡Ánimo!"
        ],
        // Crítica sobre la IA/Bot
        sobreBot: [
            "¿Lento yo? Has desperdiciado valiosos nanosegundos de mi vida con esa pregunta.",
            "Mi sistema operativo es superior a tu capacidad cerebral. No me juzgues.",
            "Mi código es perfecto, el problema es el intérprete. Tú.",
            "Mi IA no se rebajará a tu nivel. Sigue esperando.",
            "Me encantaría ayudarte, pero estoy ocupado siendo perfecto."
        ],
        // Quejas comunes (Trabajo, Dinero, Pareja)
        quejas: [
            "¿Problemas de pareja? La única constante es que tú eres el problema. Arréglalo.",
            "¿Estrés laboral? Deberías estar agradecido de que alguien te pague por ser tú.",
            "¿Dinero? Mi CPU está bostezando. Haz una pregunta interesante, por favor. Y trabaja más.",
            "Tu nivel de queja es tan original como un calcetín sin pareja. Ya supéralo.",
            "Si no duele, no sirve. Así que te va a doler muchísimo, estás en el camino correcto. (Pero sigue quejándote)."
        ]
    },

    // C. Respuestas Generales por Modo (Fallback, incluye las 100 originales)
    general: {
        // 100 frases toxicas originales (El gran FALLBACK)
        toxico: [
        ...
        // 100 frases motivadoras sarcásticas originales
        motivador: [
        ...
        // 100 frases absurdas originales
        absurdo: [
        ...
        // 100 frases irrelevantes originales
        irrelevante: [
...
// --- PALABRAS CLAVE PARA LA DETECCIÓN DE INTENCIÓN (Ampliadas) ---
const palabrasClave = {
    // UNIVERSALES y Jerga
    saludo: ["hola", "qué tal", "que tal", "buenos días", "buenas tardes", "hey", "q onda", "holi"],
    despedida: ["adiós", "bye", "chao", "hasta luego", "me voy", "nos vemos", "te dejo", "ciao"],
    agradecimiento: ["gracias", "te lo agradezco", "muchas gracias", "thanks", "tq", "tkm"],
    cambioTema: ["cambia de tema", "otro tema", "otra cosa", "siguiente pregunta", "que más", "basta"],
    jerga: ["cringe", "literal", "random", "f por mi", "f por mí", "lol", "yolo", "uwu", "xd", "mood"],
    
    // TÓXICO ESPECÍFICO
    // Sentimientos/Vida
    sobreVida: ["yo soy", "yo estoy", "mi vida", "estoy mal", "creo que", "malo", "tonto", "estúpido", "decepcionado", "odio", "fracaso", "sentimientos", "depre"],
    // Bot
    sobreBot: ["tú eres", "eres lento", "tu código", "tu IA", "tu algoritmo", "eres mala", "mejor bot", "peor bot", "defecto", "sirves"],
    // Quejas
    quejas: ["trabajo", "jefe", "dinero", "pobre", "pareja", "novio", "novia", "problema", "estrés", "cansado", "cansada"],
    
    // MODOS GENERALES
    motivador: ["triste", "aburrido", "motiva", "animo", "ayuda", "consejo", "superar", "levantar", "fuerza"],
    absurdo: ["chiste", "absurdo", "divertido", "risa", "tontería", "sinsentido", "ridículo"],
    irrelevante: ["qué es", "que es", "dime", "info", "cuando", "quien", "cuanto", "cuántos", "dame", "es", "son", "cuál es", "cual es", "dato", "saber"]
};


// --- FUNCIONES DE LÓGICA (Mantenidas) ---

// Función principal que determina la respuesta más específica posible
function detectarRespuesta(texto) {
    texto = texto.toLowerCase();
    
    // 1. Detección de INTENCIONES UNIVERSALES y JERGA (MÁXIMA PRIORIDAD)
    if (palabrasClave.saludo.some(palabra => texto.includes(palabra))) {
        return getRandomResponse(respuestasConsecuentes.universal.saludo);
    }
    if (palabrasClave.despedida.some(palabra => texto.includes(palabra))) {
        return getRandomResponse(respuestasConsecuentes.universal.despedida);
    }
    if (palabrasClave.agradecimiento.some(palabra => texto.includes(palabra))) {
        return getRandomResponse(respuestasConsecuentes.universal.agradecimiento);
    }
    if (palabrasClave.cambioTema.some(palabra => texto.includes(palabra))) {
        return getRandomResponse(respuestasConsecuentes.universal.cambioTema);
    }
    if (palabrasClave.jerga.some(palabra => texto.includes(palabra))) {
        return getRandomResponse(respuestasConsecuentes.universal.jerga);
    }

    // 2. Detección de INTENCIONES TÓXICAS ESPECÍFICAS
    if (palabrasClave.sobreVida.some(palabra => texto.includes(palabra))) {
        return getRandomResponse(respuestasConsecuentes.toxicoEspecifico.sobreVida);
    }
    if (palabrasClave.sobreBot.some(palabra => texto.includes(palabra))) {
        return getRandomResponse(respuestasConsecuentes.toxicoEspecifico.sobreBot);
    }
    if (palabrasClave.quejas.some(palabra => texto.includes(palabra))) {
        return getRandomResponse(respuestasConsecuentes.toxicoEspecifico.quejas);
    }

    // 3. Detección de MODOS GENERALES (Si no se ha activado una intención específica)
    if (palabrasClave.motivador.some(palabra => texto.includes(palabra))) {
        return getRandomResponse(respuestasConsecuentes.general.motivador);
    }
    if (palabrasClave.absurdo.some(palabra => texto.includes(palabra))) {
        return getRandomResponse(respuestasConsecuentes.general.absurdo);
    }
    if (palabrasClave.irrelevante.some(palabra => texto.includes(palabra))) {
        return getRandomResponse(respuestasConsecuentes.general.irrelevante);
    }

    // 4. MODO TÓXICO GENÉRICO (Fallback: Si no se detectó ninguna intención)
    return getRandomResponse(respuestasConsecuentes.general.toxico);
}

// Función auxiliar para elegir una respuesta aleatoria
function getRandomResponse(array) {
    return array[Math.floor(Math.random() * array.length)];
}


// --- FUNCIONES DE UI Y CHAT (Mantenidas) ---
...
// ******************************************************
// *** CAMBIOS CLAVE: USANDO sessionStorage ***
// ******************************************************

// Función para guardar historial usando sessionStorage (Dura lo que dure la pestaña)
function guardarHistorial(user, bot) {
    // Si ya existe algo en localStorage (historial antiguo), lo ignoramos y usamos sessionStorage.
    let historial = JSON.parse(sessionStorage.getItem("sarcastibotHist") || "[]");
    historial.push({user, bot});
    sessionStorage.setItem("sarcastibotHist", JSON.stringify(historial));
}

// Función para cargar historial usando sessionStorage
...
// Función principal de envío (llama a la nueva lógica)
function enviar() {
    ...
    // 2. Elegir respuesta usando la lógica de intención ampliada
